/*
 * -------------------------------------------------------------------------------------------------------------------------
 *                                    REALISTIC HEARTBEAT LED EXAMPLE (BARE METAL)
 * -------------------------------------------------------------------------------------------------------------------------
 ...
 * inverting the boolean expression in the LUT in the CCL [!(TCB_WO * (TCD_WOA ^ TCB_WOB))] or inverting the IO pin
 ...
 * must be changed if clock frequencies or clock divisions are changed
 */
...
int main(void)
{
	...
	/* Only need to be called if changing the default BPM period and pulse_length during runtime is desirable */
...
void init_heartbeat()
{
	...
	/* Enable and set system clock prescaler to get appropriate speed (see F_CPU)*/
	_PROTECTED_WRITE(CLKCTRL_MCLKCTRLB, (CLKCTRL_PDIV_32X_gc | CLKCTRL_PEN_bm));

	/* Enable TCB in 8-Bit PWM Mode with a prescaler of 2 (see F_TCB_CLK) */
	TCB0.CTRLB = TCB_CCMPEN_bm | TCB_CNTMODE_PWM8_gc;
	TCB0.CTRLA = TCB_CLKSEL_CLKDIV2_gc | TCB_ENABLE_bm;

	/* Enable TCD with a prescaler of 4 (see F_TCD_CLK) */
	TCD0.CTRLA = TCD_ENABLE_bm | TCD_CNTPRES_DIV4_gc;

	/* Set LED pin (PB4) to output for CCL LUT0 alternative pin, inverted as LED is active low */
	PORTB.DIR      = PIN4_bm;
	PORTB.PIN4CTRL = PORT_INVEN_bm | PORT_PULLUPEN_bm;
	...
	/* Enable sleep mode and set to idle mode */
	SLPCTRL.CTRLA = SLPCTRL_SEN_bm | SLPCTRL_SMODE_IDLE_gc;
}

/*
 * Sets BPM = 54 with a pulse width of 40%.
 * You can use this function if changing BPM and pulse length during runtime is not desirable.
 * See set_heartbeat_BPM() to find new default values other BPMs and pulse_lengths.
 */
void set_default_BPM()
{
